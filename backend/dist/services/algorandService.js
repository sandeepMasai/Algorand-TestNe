...
class AlgorandService {
    constructor() {
        ...
        try {
            ...
            this.algodClient = new algosdk_1.default.Algodv2(ALGOD_TOKEN || "", fullUrl);
            ...
            this.algodClient = new algosdk_1.default.Algodv2(ALGOD_TOKEN || "", cleanHostname, port);
    ...
    async testConnection() {
        try {
            ...
            if (errorMessage.includes("fetch failed") || errorMessage.includes("ECONNREFUSED")) {
                throw new Error(`Network connection failed. Cannot reach Algorand node at ${ALGOD_SERVER}. ` +
            ...
            throw new Error(`Cannot connect to Algorand network (${ALGOD_SERVER}). ` +
                `Error: ${errorMessage}`);
        }
    }
    /**
    ...
    async sendTransaction(fromMnemonic, toAddress, amount, note) {
        try {
            ...
            if (errorMessage.includes("fetch failed") || errorMessage.includes("ECONNREFUSED") || errorMessage.includes("ENOTFOUND")) {
                const networkError = new Error(`Network connection failed. Cannot reach Algorand node at ${ALGOD_SERVER}. ` +
