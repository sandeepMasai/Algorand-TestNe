...
export function useTransactions() {
    ...
    const load = useCallback(async (options) => {
        ...
        try {
            ...
            if (shouldConfirm) {
                ...
                if (pending.length) {
                    await Promise.allSettled(pending.map((txn) => confirmTransaction(txn.txId).catch((error) => {
                        if (!silent) {
                            toast.error(error instanceof Error ? error.message : `Failed to confirm ${txn.txId}`);
                        }
                        throw error;
                    })));
    ...
    const pendingCount = useMemo(() => transactions.filter((txn) => txn.status === "pending").length, [transactions]);
    useEffect(() => {
        void load({ confirm: true });
    }, [load]);
    useEffect(() => {
